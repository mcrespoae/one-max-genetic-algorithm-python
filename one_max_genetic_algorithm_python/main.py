...
def process_genetic_algorithm(mutation_rate_values: List[float], crossover_rate_values: List[float]):
    ...
    for mutation_rate in mutation_rate_values:
        ...
        if results.score >= TARGET_PROBLEM_FITNESS:  # Check if perfect score to close the algorithm execution
...
def main() -> None:
    ...
    mutation_rate_values = generate_equally_spaced_values(min_val=MUTATION_RATE_MIN, max_val=MUTATION_RATE_MAX, length=8)
