...
class TestUnitGeneticAlgorithm(unittest.TestCase):
    ...
    def test_genome_fitness_all_zeros(self):
        genome = np.zeros(10, dtype=np.int8)
    ...
    def test_genome_fitness_all_ones(self):
        genome = np.ones(10, dtype=np.int8)
        expected_fitness = 1.0
        actual_fitness = get_genome_fitness(genome)
        self.assertEqual(actual_fitness, expected_fitness)

    def test_genome_fitness_mixed(self):
        genome = np.array([0, 1, 0, 1, 0, 1, 1, 0], dtype=np.int8)
    ...
    def test_get_best_fitness_multiple_values(self):
        fitness_values = np.array([0.2, 0.9, 0.4, 0.6, 0.3])
    ...
    def test_get_generation_fitness_multiple_values_population(self):
        fitness_values = np.array([0.5, 0.7, 0.9])
    ...
    def test_crossover_no_crossover(self):
        ...
        np.testing.assert_equal(child1, parent1)
        np.testing.assert_equal(child2, parent2)
    ...
    def test_crossover_with_crossover(self):
        ...
        with np.testing.assert_raises(AssertionError):
            np.testing.assert_equal(child1, parent1)
            np.testing.assert_equal(child2, parent2)
            np.testing.assert_equal(child1, child2)

    def test_mutate_zeros_with_rate_1(self):
        np.testing.assert_equal(mutate(np.array([0, 0, 0, 0]), 1.0), np.array([1, 1, 1, 1]))

    def test_mutate_ones_with_rate_1(self):
        np.testing.assert_equal(mutate(np.array([1, 1, 1, 1]), 1.0), np.array([0, 0, 0, 0]))

    def test_mutate_mixed_with_rate_0(self):
        np.testing.assert_equal(mutate(np.array([0, 1, 0, 1]), 0.0), np.array([0, 1, 0, 1]))

    def test_mutate_mixed_with_rate_0_5(self):
        original_genome = np.array([0, 1, 0, 1])
        mutated_genome = mutate(original_genome, 0.5)
    ...
    def test_mutate_population_fitness_all_ones(self):
        ...
        mutation_rate = 0.5
        genome_length = 50
        genome = np.ones(genome_length, dtype=np.int8)
        mutated_gen_fitness = []
...
class TestInteGeneticAlgorithm(unittest.TestCase):
    ...
    def test_inte_new_population_zeroes(self):
        ...
        genome = np.zeros(genome_length, dtype=np.int8)
        population = np.tile(genome, (population_size, 1))
        fitness_values = calculate_population_fitnesses(population)
        new_population = np.empty(population.shape, dtype=np.int8)
    ...
    def test_inte_get_avg(self):
        ...
        self.assertTrue(0.06 <= avg_best_fitness <= 0.72)
